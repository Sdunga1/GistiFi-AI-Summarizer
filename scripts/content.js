...
// Create and inject floating action button (only on LeetCode pages)
async function createFloatingButton() {
  ...
  // Only show floating button on LeetCode pages
  if (!window.location.href.includes('leetcode.com')) {
    console.log('Not a LeetCode page, skipping floating button creation');
    return;
  }
      ...
      const handleAsync = async () => {
        try {
          ...
          if (req.type === 'SIDE_PANEL_CLOSED') {
            ...
            // Create a new button (only on LeetCode pages)
            if (window.location.href.includes('leetcode.com')) {
  ...
  if (!document.hidden) {
    ...
    try {
      ...
      if (response && response.isOpen) {
        ...
        // Side panel is closed, ensure button exists (only on LeetCode pages)
        if (
          !document.querySelector('.gistifi-fab-container') &&
